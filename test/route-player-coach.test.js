...
describe("POST /api/player-coach — account mode", () => {
  ...
  it("403s a coach-parent who is acting as the parent (the worn hat decides)", async () => {
    coachParentSession();
    const { POST } = await loadRoute({ authOn: true });
    const res = await POST(fakeRequest({ body: { playerId: "p1", ratings: RATINGS }, cookies: { act_as: "parent" } }));
    expect(res.status).toBe(403);
    expect(await res.json()).toEqual({ error: "forbidden" });
    expect(setData).not.toHaveBeenCalled();
    // Without the act_as cookie the same login wears the coach hat and may write.
    expect((await POST(fakeRequest({ body: { playerId: "p1", ratings: RATINGS } }))).status).toBe(200);
  });
