...
import { occurrences, isoLocal, addDays, activeOn, isCarnival, carnivalDescription } from "@/lib/dashboardData";
...
function gameEv(f, team) {
  ...
  return { uid: f.id, title: cx + "⚽ " + title, dateISO: f.dateISO, time: f.time, endTime: f.time ? addMin(f.time, 105) : null, location: f.venue, desc: `Round ${f.round}${res}${f.status === "cancelled" ? " — CANCELLED" : ""}`, allDay: !f.time };
}
// A carnival is ONE timed event across its whole window (start to end, six
// hours when no end is set) titled "Carnival: …", its games listed in the
// description — never one event per game. components/Dashboard.jsx's
// sessionEv mirrors this so the sheet's Add-to-calendar matches the feed.
...
export function seasonICS(data) {
  ...
  (data?.sessions || []).forEach((s) => {
    ...
    else years.forEach((y) => occurrences(s, y).forEach((iso) => body.push(...vevent(sessionEv(s, iso)))));
  });
