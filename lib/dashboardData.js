...
// date with a start/end window, a venue and the day's games
// ({ id, time, opponent, pitch }). Everything else about it (occurrences,
// the per-occurrence RSVP store, the ICS one-off) is the ordinary one-off
// session machinery. `games` may be empty while the schedule is unpublished.
export const isCarnival = (s) => s?.kind === "carnival";
// The day's games in time order; a row without an opponent is never shown.
...
export function carnivalMeta(s) {
  const n = carnivalGames(s).length;
...
// then the notes.
export function carnivalDescription(s) {
  const lines = carnivalGames(s).map(g => `${g.time || "Time TBC"} vs ${String(g.opponent).trim()}${pitchLabel(g.pitch) ? ` (${pitchLabel(g.pitch)})` : ""}`);
