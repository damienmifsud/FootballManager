...
import { teamSeason } from "@/lib/teamSetup";

export const isoLocal = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
// The season is no longer a constant: teamSeason(data.team) (lib/teamSetup)
// gives each team its own window, defaulting to the current calendar year.
...
// Carnival games (fixtures linked to a carnival session) are friendlies for
// the day and never count: not in the record, form, goals, rounds or scorers.
export function computeStats(data) {
  const played = data.fixtures.filter(f => f.status === "played" && f.us != null && f.them != null && !isCarnivalGame(data, f));
...
// The soonest upcoming league fixture (ignores past/played and carnival
// games — the Home Carnival card covers those; TBC dates sort last).
export function nextFixture(data) {
  const today = isoLocal(new Date());
  const up = data.fixtures
    .filter(f => f.status === "upcoming" && (!f.dateISO || f.dateISO >= today) && !isCarnivalGame(data, f))
...
// ---- Carnivals -------------------------------------------------------------
// A carnival is a session of kind "carnival": a one-off (recur "once") on one
// date with a start/end window and a venue. Its games are ordinary fixtures
// that carry `carnivalId` (the carnival's id): they show in Results tagged with
// the carnival, never on the calendar grid (the carnival badge covers the day)
// and never in the season record. Everything else about a carnival
// (occurrences, the per-occurrence RSVP store, the ICS one-off) is the
// ordinary one-off session machinery. A fixture whose carnivalId names no
// existing carnival is a normal fixture.
export const isCarnival = (s) => s?.kind === "carnival";
// The carnival a fixture belongs to, or null.
export const carnivalOf = (data, f) => (f?.carnivalId && (data?.sessions || []).find(s => isCarnival(s) && s.id === f.carnivalId)) || null;
export const isCarnivalGame = (data, f) => !!carnivalOf(data, f);
// The fixtures linked to a carnival, by time then opponent.
export const carnivalFixtures = (data, c) => !c ? [] : (data?.fixtures || [])
  .filter(f => f.carnivalId === c.id)
  .sort((a, b) => (a.time || "99:99").localeCompare(b.time || "99:99") || String(a.opponent || "").localeCompare(String(b.opponent || "")));
// The retired hand-typed list ({ id, time, opponent, pitch }), in time order;
// only a fallback for carnivals saved before games became fixtures.
export const carnivalGames = (s) => (Array.isArray(s?.games) ? s.games : [])
  .filter(g => g && String(g.opponent || "").trim())
  .slice()
  .sort((a, b) => (a.time || "99:99").localeCompare(b.time || "99:99"));
// The rows the Games card, row meta and calendar description show: the linked
// fixtures (each row carries `fixture`), else the old list read-only.
export function carnivalGameRows(s, data) {
  const linked = carnivalFixtures(data, s);
  if (linked.length) return linked.map(f => ({ id: f.id, time: f.time || "", opponent: String(f.opponent || "").trim(), pitch: String(f.pitch || "").trim(), fixture: f }));
  return carnivalGames(s);
}
// "Pitch 4" for a bare "4"; anything the coach typed in full is shown as is.
export const pitchLabel = (pitch) => {
  const t = String(pitch || "").trim();
  return !t ? "" : /^\d+[a-z]?$/i.test(t) ? `Pitch ${t}` : t;
};
// "08:00–14:00" (en dash, no spaces) for row meta and the Home card.
export const timeWindow = (s) => s?.time ? (s.endTime ? `${s.time}–${s.endTime}` : s.time) : "";
// Row meta: "3 games · Perry Park · 08:00–14:00" / "Games to be announced · …".
export function carnivalMeta(s, data) {
  const n = carnivalGameRows(s, data).length;
  return [n ? `${n} ${n === 1 ? "game" : "games"}` : "Games to be announced", s.location, timeWindow(s)].filter(Boolean).join(" · ");
}
// Calendar description: one line per game ("08:30 vs Oxley United (Pitch 4)",
// "· 3–1" appended once played), then the notes.
export function carnivalDescription(s, data) {
  const lines = carnivalGameRows(s, data).map(g => {
    const f = g.fixture;
    const score = f && f.status === "played" && f.us != null && f.them != null ? ` · ${f.us}–${f.them}` : "";
    return `${g.time || "Time TBC"} vs ${g.opponent}${pitchLabel(g.pitch) ? ` (${pitchLabel(g.pitch)})` : ""}${score}`;
  });
  const notes = String(s.notes || "").trim();
  if (notes) { if (lines.length) lines.push(""); lines.push(notes); }
  return lines.join("\n");
}

// Expand a training/activity session into its dates within `year`. A weekly
// session is clamped to `window` ({ startISO, endISO } — the team's season) as
// well as to the year; its own startISO/untilISO can narrow that further. A
// one-off is an explicit date and is only matched on year.
export function occurrences(s, year = thisYear(), window = null) {
  if (!s) return [];
  if (s.recur !== "weekly") {
    if (!s.dateISO) return [];
    return new Date(s.dateISO + "T00:00:00").getFullYear() === year ? [s.dateISO] : [];
  }
  let lo = `${year}-01-01`, hi = `${year}-12-31`;
  if (window?.startISO && window.startISO > lo) lo = window.startISO;
  if (window?.endISO && window.endISO < hi) hi = window.endISO;
  if (s.startISO && s.startISO > lo) lo = s.startISO;
  if (s.untilISO && s.untilISO < hi) hi = s.untilISO;
  if (lo > hi) return [];
  const to = new Date(hi + "T00:00:00");
  const out = [];
  const d = new Date(lo + "T00:00:00");
  while (d.getDay() !== (s.weekday ?? 2)) d.setDate(d.getDate() + 1);
  let guard = 0; // an 18-month season is under 80 weeks
  while (d <= to && guard++ < 120) { out.push(isoLocal(d)); d.setDate(d.getDate() + 7); }
  return out;
}
...
export function monthItems(data, year, month) {
  ...
  const items = [];
  // A carnival game is part of the carnival row, never its own row or crest.
  data.fixtures.forEach(f => {
    if (!f.dateISO || isCarnivalGame(data, f)) return;
  ...
  (data.sessions || []).forEach(s => {
    occurrences(s, year, season).forEach(iso => {
...
export function upcomingItems(data, fromISO, days = 7) {
  ...
  const items = [];
  (data.fixtures || []).forEach(f => {
    if (f.dateISO && inRange(f.dateISO) && !isCarnivalGame(data, f))
      items.push({ key: f.id, dateISO: f.dateISO, time: f.time, kind: "game", title: "vs " + f.opponent, ref: f });
  });
  const years = new Set([+fromISO.slice(0, 4), +lastISO.slice(0, 4)]);
  (data.sessions || []).forEach(s => {
    const seen = new Set();
    years.forEach(y => occurrences(s, y, season).forEach(iso => {
