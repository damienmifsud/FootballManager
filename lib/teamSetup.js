...
// flags, what parents see, match-day rules, coach-only player fields, the
// match format and the season window. Everything lives on the team document (data.team.*) so the
...
// ---- Season window ---------------------------------------------------------
// The dates a team's season runs between. Weekly training only expands inside
// this window and the calendar tab only navigates across it, so seeded
// sessions no longer show "forever" and a 2027 (or a summer 2026/27) season
// is just a matter of setting the dates. Stored as team.season =
// { startISO, endISO }; unset means "follow the current calendar year".
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
const isoOf = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
const validISO = (s) => typeof s === "string" && DATE_RE.test(s) && isoOf(new Date(s + "T00:00:00")) === s;
const SEASON_MAX_MONTHS = 18;

// Server-side guard: both dates valid, start on or before end, and the span
// no longer than 18 months. Anything else is "unset" (null).
export function sanitizeSeason(input) {
  if (!input || typeof input !== "object") return null;
  const startISO = String(input.startISO || "").trim();
  const endISO = String(input.endISO || "").trim();
  if (!validISO(startISO) || !validISO(endISO) || startISO > endISO) return null;
  const cap = new Date(startISO + "T00:00:00");
  cap.setMonth(cap.getMonth() + SEASON_MAX_MONTHS);
  if (endISO > isoOf(cap)) return null;
  return { startISO, endISO };
}

// The effective window for a team: its stored season, else the whole calendar
// year of `todayISO` (so an unset team follows the current year and rolls over
// on 1 January).
export function teamSeason(team, todayISO = isoOf(new Date())) {
  const stored = sanitizeSeason(team?.season);
  if (stored) return stored;
  const year = String(todayISO || isoOf(new Date())).slice(0, 4);
  return { startISO: `${year}-01-01`, endISO: `${year}-12-31` };
}

// "2026" when the window sits inside one year, else "2026/27".
export function seasonLabel(season) {
  const y1 = String(season?.startISO || "").slice(0, 4);
  const y2 = String(season?.endISO || "").slice(0, 4);
  if (!y1 || !y2 || y1 === y2) return y1 || y2;
  return `${y1}/${y2.slice(2)}`;
}

// The distinct calendar years the window touches, ascending.
export function seasonYears(season) {
  const y1 = parseInt(String(season?.startISO || "").slice(0, 4), 10);
  const y2 = parseInt(String(season?.endISO || "").slice(0, 4), 10);
...
// Every { year, month } (month 0-11) from the start month to the end month
// inclusive — the months the calendar tab can show.
export function seasonMonths(season) {
  const s = String(season?.startISO || ""), e = String(season?.endISO || "");
  if (!DATE_RE.test(s) || !DATE_RE.test(e) || s > e) return [];
  const out = [];
  let year = +s.slice(0, 4), month = +s.slice(5, 7) - 1;
  const endYear = +e.slice(0, 4), endMonth = +e.slice(5, 7) - 1;
