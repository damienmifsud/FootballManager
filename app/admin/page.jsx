...
// Club access control — super admin only. Roles are identities (emails), so
...
export default async function AdminPage() {
  ...
  const { isAdminEmail } = await import("@/lib/directory");
  ...
  if (!isAdminEmail(email)) {
    return (
      <div style={wrap}>
        <div style={{ maxWidth: 400 }}>
          <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Super admins only</div>
          <div style={{ fontSize: 14, opacity: .9 }}>{email} isn't listed in ADMIN_EMAILS.</div>
        </div>
      </div>
    );
  }
  ...
  return <AccessManager adminEmail={email} />;
}
